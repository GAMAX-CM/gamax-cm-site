let API_BASE = "http://localhost:3000"; // fallback local
...
function persistCart(){ localStorage.setItem('gamax_cart', JSON.stringify(cart)); }

function updateConfigPrice(){
  const type = typeEl.value;
  const l = parseFloat(largeurEl.value) || 0;
  const L = parseFloat(longueurEl.value) || 0;
  const h = parseFloat(hauteurEl.value) || 0;
  const base = type === 'abri' ? 1500 : (type === 'hangar' ? 2800 : 1200);
  const price = base * Math.max(1, (l * L * h) / 50);
